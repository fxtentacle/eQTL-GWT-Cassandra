...
public class ColumnPath implements TBase, java.io.Serializable, Cloneable, Comparable<ColumnPath> {
	 ...
	 */
...

		{
			...
			put(SUPER_COLUMN, new FieldMetaData("super_column", TFieldRequirementType.OPTIONAL, new FieldValueMetaData(TType.STRING)));
			put(COLUMN, new FieldMetaData("column", TFieldRequirementType.OPTIONAL, new FieldValueMetaData(TType.STRING)));
		}
	});
	...
	// Returns true if field column_family is set (has been asigned a value) and
	// false otherwise
		...
		if (isSetSuper_column()) {
			...
			if (this.super_column == null) {
				...
				for (int i = 0; i < __super_column_size; i++) {
					if (i != 0)
						sb.append(" ");
					sb.append(Integer.toHexString(this.super_column[i]).length() > 1 ? Integer.toHexString(this.super_column[i]).substring(Integer.toHexString(this.super_column[i]).length() - 2)
							.toUpperCase() : "0" + Integer.toHexString(this.super_column[i]).toUpperCase());
				}
		...
		if (isSetColumn()) {
			...
			if (this.column == null) {
				...
				for (int i = 0; i < __column_size; i++) {
					if (i != 0)
						sb.append(" ");
					sb.append(Integer.toHexString(this.column[i]).length() > 1 ? Integer.toHexString(this.column[i]).substring(Integer.toHexString(this.column[i]).length() - 2).toUpperCase() : "0"
							+ Integer.toHexString(this.column[i]).toUpperCase());
				}
