 ...
 * http://en.wikipedia.org/wiki/Predicate_(mathematical_logic)), which is
 * described as "a property that the elements of a set have in common."
 * 
 * SlicePredicate's in Cassandra are described with either a list of
 * column_names or a SliceRange. If column_names is specified, slice_range is
 * ignored.
 * 
 * @param column_name
 *            . A list of column names to retrieve. This can be used similar to
 *            Memcached's "multi-get" feature to fetch N known column names. For
 *            instance, if you know you wish to fetch columns 'Joe', 'Jack', and
 *            'Jim' you can pass those column names as a list to fetch all three
 *            at once.
 * @param slice_range
 *            . A SliceRange describing how to range, order, and/or limit the
 *            slice.
 */
public class SlicePredicate implements TBase, java.io.Serializable, Cloneable, Comparable<SlicePredicate> {
	 ...
	 */
		...
		{
			put(COLUMN_NAMES, new FieldMetaData("column_names", TFieldRequirementType.OPTIONAL, new ListMetaData(TType.LIST, new FieldValueMetaData(TType.STRING))));
			put(SLICE_RANGE, new FieldMetaData("slice_range", TFieldRequirementType.OPTIONAL, new StructMetaData(TType.STRUCT, SliceRange.class)));
		}
	});
	...
	// Returns true if field column_names is set (has been asigned a value) and
	// false otherwise
	...
	// Returns true if field slice_range is set (has been asigned a value) and
	// false otherwise
