 ...
 * 
 * @param start
 *            . The column name to start the slice with. This attribute is not
 *            required, though there is no default value, and can be safely set
 *            to '', i.e., an empty byte array, to start with the first column
 *            name. Otherwise, it must a valid value under the rules of the
 *            Comparator defined for the given ColumnFamily.
 * @param finish
 *            . The column name to stop the slice at. This attribute is not
 *            required, though there is no default value, and can be safely set
 *            to an empty byte array to not stop until 'count' results are seen.
 *            Otherwise, it must also be a value value to the ColumnFamily
 *            Comparator.
 * @param reversed
 *            . Whether the results should be ordered in reversed order. Similar
 *            to ORDER BY blah DESC in SQL.
 * @param count
 *            . How many keys to return. Similar to LIMIT 100 in SQL. May be
 *            arbitrarily large, but Thrift will materialize the whole result
 *            into memory before returning it to the client, so be aware that
 *            you may be better served by iterating through slices by passing
 *            the last value of one call in as the 'start' of the next instead
 *            of increasing 'count' arbitrarily large.
 */
public class SliceRange implements TBase, java.io.Serializable, Cloneable, Comparable<SliceRange> {
	 ...
	 */
...

		{
			...
			put(REVERSED, new FieldMetaData("reversed", TFieldRequirementType.REQUIRED, new FieldValueMetaData(TType.BOOL)));
	...
	// Returns true if field start is set (has been asigned a value) and false
	// otherwise
	...
	// Returns true if field finish is set (has been asigned a value) and false
	// otherwise
	...
	// Returns true if field reversed is set (has been asigned a value) and
	// false otherwise
	...
	public String toString() {
		...
		if (this.start == null) {
			...
			for (int i = 0; i < __start_size; i++) {
				if (i != 0)
					sb.append(" ");
				sb.append(Integer.toHexString(this.start[i]).length() > 1 ? Integer.toHexString(this.start[i]).substring(Integer.toHexString(this.start[i]).length() - 2).toUpperCase() : "0"
						+ Integer.toHexString(this.start[i]).toUpperCase());
			}
		...
		if (this.finish == null) {
			...
			for (int i = 0; i < __finish_size; i++) {
				if (i != 0)
					sb.append(" ");
				sb.append(Integer.toHexString(this.finish[i]).length() > 1 ? Integer.toHexString(this.finish[i]).substring(Integer.toHexString(this.finish[i]).length() - 2).toUpperCase() : "0"
						+ Integer.toHexString(this.finish[i]).toUpperCase());
			}
	...
	public void validate() throws TException {
		...
		// alas, we cannot check 'reversed' because it's a primitive and you
